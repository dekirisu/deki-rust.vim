...
sy match rsEnumVarAssumed '\v([A-Z][A-Za-z0-9]*::(\<.*\>::)*)@<=[A-Z][A-Za-z0-9]*'
sy match rsImplType '\v(Self::)@<=[A-Z][A-Za-z0-9]*'
...
sy keyword rsBuiltin i8 u8 i16 u16 i32 u32 i64 u64 i128 u128 isize usize str f32 f64 bool 
...
syntax match rsUserMacroIdent '\v#[a-z][a-z0-9_]*'

" macro_rules! uses $foobar for parameters
syntax match rsUserMacroIdent '\v\$[a-z][a-z0-9_]*'
syntax match rsUserMacroIdent '\v\$[a-z][a-z0-9_]*:[a-z]*'
syntax match rsKnownUnused '\v_[a-z0-9_]+'
...
syntax match rsComparison '[&\|<=>!]\+'
syntax match rsEqual '\(/\|*\|+\||\|&\|-\)='
...
hi default link rsImplType Type 
